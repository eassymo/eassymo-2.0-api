...
def insert(payload) -> CallCenterManagementList:
    try:
        inserted_id = database.db["CallCenterManagementList"].insert_one(payload).inserted_id

        call_center_data = database.db["CallCenterManagementList"].find_one({"_id": inserted_id})
...
def find(filters) -> List[CallCenterManagementList]:
    try:
        callcenters_data = list(database.db["CallCenterManagementList"].find(filters))

        call_center_management_lists: List[CallCenterManagementList] = []

        for callcenter_management_list_data in callcenters_data:
            print(callcenter_management_list_data)
            call_center_management_lists.append(CallCenterManagementList(**callcenter_management_list_data))
...
def find_by_id(list_id: str) -> CallCenterManagementList:
    try:
        ...
        callcenter_data = database.db["CallCenterManagementList"].find_one({"_id": ObjectId(list_id)})
        
        if not callcenter_data:
            return None
        
        if "groups" in callcenter_data and callcenter_data["groups"]:
            group_ids = [ObjectId(group_id) for group_id in callcenter_data["groups"] if isinstance(group_id, str)]
            
            if group_ids:
                group_info = list(database.db["groups"].find({"_id": {"$in": group_ids}}))
