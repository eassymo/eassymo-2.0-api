...
def find(filters) -> List[Dict[str, Any]]:
    try:
        callcenter_connections = CallCenterConnectionRepository.find(filters)

        formatted_connections = []
        if len(callcenter_connections) > 0:

            for callcenter_connection in callcenter_connections:
                callcenter_info = GroupRepository.find_by_id(
                    callcenter_connection.callcenter_id)
                ...
                formatted_connections.append(callcenter_connection.toJson())
...
def delete(id: str) -> Dict[str, Any]:
    try:
        callcenter_connection = CallCenterConnectionRepository.delete(
            {"_id": ObjectId(id)})
